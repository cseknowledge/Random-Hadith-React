...
export default function Example() {
  ...
  const getHadith = async () => {
    await axios.get('https://api.sunnah.com/v1/hadiths/random', config)
    .then(res => {
      const hadithText = res.data.data[0].hadith;
      setHadith(parse(hadithText));
      setLoading(false);
  ...
  useEffect(() => {
    ...
    let intervalId;
    if (autoFetch) {
      intervalId = setInterval(getHadith, 60000);
    } else {
    ...
    <>
      <div className="min-h-full flex flex-auto h-screen bg-hero-pattern align-middle">
        <div class="w-full h-2/3 m-auto ml-8 mr-8 p-4 text-center bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
          <h5 class="mb-2 text-3xl font-bold text-blue-900 dark:text-white">Random Hadith</h5>
          <p class="mb-5 text-base text-gray-500 sm:text-lg dark:text-gray-400">Hadith will show here...</p>
          ...
          <div class="items-center justify-center space-y-4 sm:flex sm:space-y-0 sm:space-x-4">
            ...
            <button type="button" onClick={()=>getHadith()} class="absolute right-16 bottom-32 px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            ...
            {/* <button type="button" class="absolute right-16 bottom-32 px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
